    ...
    execute(argument, message, EmbedBuilder, client, typeofcommand, database_service) {
        if (typeofcommand === "message") {
            ...
            if (message.options.getSubcommand() === "user") {
                ...
                fetch(`https://users.roblox.com/v1/usernames/users`, { method: "POST", body: JSON.stringify({ "usernames": [infomation], "excludeBannedUsers": false }), headers: { 'Content-Type': 'application/json' } }).then(res => Promise.all([res.status, res.json()])).then(async ([status_1, json]) => {
                    if (!json) return message.editReply({ embeds: [new EmbedBuilder().setDescription(`<:PoxError:1025977546019450972> Can't fetch user profile, maybe api cooldown.`).setColor(`Red`)] });
                    if (status_1 === 500) return message.editReply({ embeds: [new EmbedBuilder().setDescription(`<:PoxError:1025977546019450972> Can't fetch user profile, Api cooldown.`).setColor(`Red`)] }); // prevent Roblox downtime joke
                    ...
                    fetch(`https://users.roblox.com/v1/users/${UserId}`).then(resinfo => resinfo.json()).then(async jsoninfo => {
                        if (UserId === null) {
                            ...
                            if (UserId === null) {
                                return message.editReply({ embeds: [new EmbedBuilder().setDescription(`<:PoxError:1025977546019450972> Invalid Roblox user!\n"${infomation}"\n${UserId}`).setColor(`Red`)] });
                            }
                        }
                        fetch(`https://presence.roblox.com/v1/presence/last-online`, { method: "POST", body: JSON.stringify({ "userIds": [UserId] }), headers: { 'Content-Type': 'application/json' }}).then(resonline => resonline.json()).then(jsononline => {
                            if (jsononline["lastOnlineTimestamps"]) {
                                if (!jsononline["lastOnlineTimestamps"][0]) return message.editReply({ embeds: [new EmbedBuilder().setDescription(`<:PoxError:1025977546019450972> Invalid Roblox user!`).setColor(`Red`)] });
                                if (!jsononline["lastOnlineTimestamps"][0]["userId"] || !jsononline["lastOnlineTimestamps"][0]["lastOnline"]) return message.editReply({ embeds: [new EmbedBuilder().setDescription(`<:PoxError:1025977546019450972> Invalid Roblox user!`).setColor(`Red`)] });
                                let LastOnlineMoment = moment(new Date(jsononline["lastOnlineTimestamps"][0]["lastOnline"]).getTime()).fromNow();
                                fetch(`https://thumbnails.roblox.com/v1/users/avatar-headshot?userIds=${UserId}&size=100x100&format=Png&isCircular=false`).then(resavatarthumbnail => resavatarthumbnail.json()).then(jsavatarthumbnail => {
                                    if (!jsavatarthumbnail["errors"]) {
                                        if (!json["errors"]) {
                                        ...
                                        return message.editReply({ embeds: [new EmbedBuilder().setDescription(`<:PoxError:1025977546019450972> Invalid Roblox user!`).setColor(`Red`)] });
                                    }
                                })
                            } else {
                                return message.editReply({ embeds: [new EmbedBuilder().setDescription(`<:PoxError:1025977546019450972> Invalid Roblox user!`).setColor(`Red`)] });
                ...
                if (!infomation) return message.editReply({ embeds: [new EmbedBuilder().setDescription(`<:PoxError:1025977546019450972> Invalid gameId/universeId!`).setColor(`Red`)] });
                infomation = infomation?.toString().trim();
                fetch(`https://apis.roblox.com/universes/v1/places/${infomation}/universe`).then(res1 => res1.json()).then(jsonid => {
                    if (!jsonid) return message.editReply({ embeds: [new EmbedBuilder().setDescription(`<:PoxError:1025977546019450972> Invalid gameId/universeId1!`).setColor(`Red`)] });
                    if (!jsonid["universeId"]) return
                    ...
                    fetch(`https://thumbnails.roblox.com/v1/games/icons?universeIds=${universeId},0&returnPolicy=PlaceHolder&size=128x128&format=Png&isCircular=false`).then(resav => resav.json()).then(jsonthumbnail => {
                        if (!jsonthumbnail) return message.editReply({ embeds: [new EmbedBuilder().setDescription(`<:PoxError:1025977546019450972> Invalid gameId/universeId2!`).setColor(`Red`)] });
                        if (!jsonthumbnail["data"] || jsonthumbnail["errors"]) return message.editReply({ embeds: [new EmbedBuilder().setDescription(`<:PoxError:1025977546019450972> Invalid gameId/universeId3!`).setColor(`Red`)] });
                        if (!jsonthumbnail["data"][0]) return message.editReply({ embeds: [new EmbedBuilder().setDescription(`<:PoxError:1025977546019450972> Invalid gameId/universeId4!`).setColor(`Red`)] });
                        if (!jsonthumbnail["data"][0]["imageUrl"]) return message.editReply({ embeds: [new EmbedBuilder().setDescription(`<:PoxError:1025977546019450972> Invalid gameId/universeId5!`).setColor(`Red`)] });
                        ...
                        fetch(`https://games.roblox.com/v1/games?universeIds=${universeId}`).then(res => res.json()).then(json => {
                            ...
                            if (jsondata && !jsondata["errors"] && jsondata["name"] !== undefined && jsondata["description"] !== undefined) {
                                ...
                                message.editReply({ embeds: [new EmbedBuilder().setTitle(`${jsondata.name}`).setThumbnail(`${jsonthumbnail["data"][0]["imageUrl"] || undefined}`).setDescription(`Description: ${getDescription()}\nGameId: ${jsondata.id}\nRoot PlaceId: ${jsondata.rootPlaceId}\nCreator: ${jsondata.creator.name}\nEdit Permission: ${jsondata.copyingAllowed}\nPlaying: ${jsondata.playing}\nVisits: ${jsondata.visits}\nMax Players: ${jsondata.maxPlayers}\nCreated: ${jsondata.created} | ${CreateMoment}\nUpdate: ${jsondata.updated} | ${LastUpdateMoment}\nAllow Private Server: ${jsondata.createVipServersAllowed}\nGame Avatar: ${jsondata.universeAvatarType}\nGenre: ${jsondata.genre}\nFavorite: ${jsondata.favoritedCount}`).setColor(`Blue`)] });
                            }else {
                                message.editReply({ embeds: [new EmbedBuilder().setDescription(`<:PoxError:1025977546019450972> Invalid gameId/universeId6!`).setColor(`Red`)] });
                            }
                        })
                    })
                })
