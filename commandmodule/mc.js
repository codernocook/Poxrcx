        ...
        if (typeofcommand === "message") {
            ...
            if (subcommand === "player") {
                ...
                fetch(`https://api.mcsrvstat.us/2/${server}`).then(res => res.json()).then(json => {
                    if (json["debug"] && json["debug"]["srv"] === false) return message.channel.send({ embeds: [new EmbedBuilder().setDescription(`<:PoxError:1025977546019450972> You sent an invaild server ip address.`).setColor(`Red`)] })
            ...
            if (message.options.getSubcommand() === "player") {
                ...
                fetch(`https://api.mcsrvstat.us/2/${server}`).then(res => res.json()).then(json => {
                    if (json["debug"] && json["debug"]["srv"] === false) return message.reply({ embeds: [new EmbedBuilder().setDescription(`<:PoxError:1025977546019450972> You sent an invaild server ip address.`).setColor(`Red`)] })
                    const serverip = json["ip"]
                    const port = json["port"]
                    const onlinemode = json["online"]
                    const playermodule = json["players"]
